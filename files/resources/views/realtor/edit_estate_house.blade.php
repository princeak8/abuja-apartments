...
<div class="form_cont">
	<h3>
		<a href="{{url('realtor/estate/'.$house->estate_id)}}">
			<?php echo $house->estate->name; ?>
    	</a>
    </h3>
	...
	{!! Form::model($house, ['action' => ['Realtor\HouseController@update'], 'method'=>'PATCH']) !!}
	  	...
	  	<div class="form-group">  
    		...
    		<input id="rent" type="radio" name="status" value="rent" required @if($house->status=='rent' || (old('status')=='rent')) checked @endif />
	    	
	    	{!! Form::label('sale', 'SALE:', ['class'=>'burgundy']) !!}
  			<input id="sale" type="radio" name="status" value="sale" required @if($house->status=='sale' || (old('status')!='rent')) checked @endif />
	  	</div>  
